...
{
    public class Skeleton : UnityEngine.MonoBehaviour
    {
        #region Enumerations
        ...
        {
            var scaleFactor = (FindBoneScaleFactor(BodyParts.LeftForeArm)
            ...
            foreach (var entry in jointCache)
            {
                var ls = entry.Value.originalLocalScale;
        ...
        private void ScaleBone(
            ...
            float size)
        {
            var renderer = bodyPart.bodyPart.gameObject.GetComponent<MeshRenderer>();
            if (renderer != null)
            {
                var distance = Vector3.Distance(end, start);
                var scale = distance / size;
                ...
                var origScale = bodyPart.originalLocalScale;
        ...
        private void AttachToJointTransformation(
            ...
            BoneTransformation bodyPart)
        {
            var attachedToJoint = bodyPart.joints[0];
            var delta = initialBodyPartPositions[bodyPartKey] - initialJointPositions[attachedToJoint];
        ...
        private void AttachToPositionJointAndMiddleJointsTransformation(
            ...
            BoneTransformation bodyPart)
        {
            var singleJoint = bodyPart.joints[0];
            var middleJoint1 = bodyPart.joints[1];
            var middleJoint2 = bodyPart.joints[2];
            var singleJointPosition = jointCache[singleJoint].transformation.position;
            var middleJointPosition = (jointCache[middleJoint1].transformation.position + jointCache[middleJoint2].transformation.position) / 2.0f;
        ...
        {
            var fromJoint = bodyPart.joints[0];
            var toJoint = bodyPart.joints[1];
            var start = jointCache[fromJoint].transformation.position;
            var end = jointCache[toJoint].transformation.position;
            var axis = bodyPart.aligningAxis;
            var size = bodyPart.originalSize;
        ...
        private void ScaleBetweenJointAndMiddleJointsTransformation(BoneTransformation bodyPart)
        {
            ...
            var middleJoint2 = bodyPart.joints[2];
            var start = jointCache[fromJoint].transformation.position;
            var end = (jointCache[middleJoint1].transformation.position + jointCache[middleJoint2].transformation.position) / 2;
            var axis = bodyPart.aligningAxis;
            var size = bodyPart.originalSize;
        ...
        private float FindBoneScaleFactor(BodyParts b)
        {
            ...
            {
                var bodyPart = bodyPartCache[b].Find(item => item.action == BoneTransformation.Transformations.ScaleBetweenJoints || item.action == BoneTransformation.Transformations.ScaleBetweenJointAndMiddleJoints);
                if (bodyPart != null)
                {
                    var fromJoint = bodyPart.joints[0];
                    var toJoint = bodyPart.joints[1];
